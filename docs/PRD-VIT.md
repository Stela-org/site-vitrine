...
| 25/07/2026 | VIT-8 PDF | **Micro-lot post-clôture : PDF du lead magnet.** Guide « Le Guide Google du commerçant local » rédigé (contenu ORIGINAL réorganisé à partir du guide + piliers du site, auteur **Corentin Janin**, charte Stela) : source `scripts/guide-google-commercant-local.html`, rendu en PDF 8 pages A4 via Playwright (`scripts/build-guide-pdf.mjs`, `npm run build:guide`) → `public/guides/guide-google-commercant-local.pdf` (492 Ko, servi en statique). `api/lead-confirm.js` : `LEAD_GUIDE_URL` a désormais pour **défaut** cette URL statique → seules `LEAD_HMAC_SECRET` + `RESEND_API_KEY` restent nécessaires (toutes deux déjà présentes en prod). Doc `docs/LEAD-MAGNET.md` mise à jour. Gate complet vert (lint:copy 62, check:brand, audit 0, build 43, check:links 2876/0, check:schema 153, check:stripe, check:alt 28). |
| 25/07/2026 | VIT-9 | **Finition réelle de la vitrine** sur `lot-vit-9`, en 4 increments à points d'arrêt. **Inc.1 (nav + bugs + comparatif)** : nav no-JS refondue en checkbox-hack (fix nav desktop invisible sous Chromium moderne / closed `<details>`), footer 4 colonnes, `ClosingBlock` global (essai + rendez-vous `calendarUrl`), `/comparatif` générique (rename `/stela-vs-dokaa`, 301). **Inc.2 (copy + intégrations + preuves + flash cookies)** : positionnement dosé, conformité recadrée « en privé d'abord » (avis publics libres), Nova nommée, `CHANNELS` (25) + `Integrations` (cartes sobres, sans logos tiers), `ROICard` honnête (établissement de démonstration), bannière cookies **cachée par défaut dans le markup** (`hidden`) montrée par script si aucun choix → **fix du flash** (test `check:cookies` durci JS-off). **Inc.3 (design/motion)** : hero dé-martelé (1 seule mention conformité, kicker/h1/lead bénéfice), hero « wahou » (toast transform-only + tilt 3D), view transitions natives 100 % CSS, `Breadcrumb` (visible + `BreadcrumbList`) sur toutes les intérieures, passe affordance, slots vidéo config-activés (silencieux tant que vide). **Inc.4 (lead magnet + légal)** : passage en **simple opt-in** (fin du double opt-in, `api/lead-confirm.js` + page `confirme` supprimés), formulaire **prénom + email + 1 case obligatoire non pré-cochée** (sans la case : ni envoi ni lead) ; logique centralisée dans `lib/leads.js` ; **2 emails à la charte** (crème, bouton laiton, logo image hébergée jamais le glyphe en texte, salutation au prénom) : guide immédiat + relance **J+3 planifiée** (`scheduled_at` Resend, sans cron) ; **poussée CRM** `POST app.mystela.fr/api/leads` `Authorization: Bearer ${LEADS_INGEST_SECRET}` (env), payload `{email, first_name, source:"guide-google", marketing_consent, consented_at}`, **fire-and-forget + 1 retry** (n'interrompt jamais le guide) ; **désinscription HMAC** en un clic (`api/lead-unsubscribe.js` → CRM `marketing_consent:false`, page `/desinscrit` noindex) ; **rate limit** Upstash REST (3/h par email, 10/h par IP, se désactive proprement si non configuré) ; **guide resserré à 7 pages** avec closing renforcé (offre + essai 7 jours) ; **politique de confidentialité** enrichie (section guide/CRM + sous-traitants nommés Vercel/Stripe/Resend/Twilio/Google/GA4/PostHog + section données Google API « Limited Use »). Gate complet vert : lint:copy + check:brand + audit 0 + build **45 pages** + check:links **1986/0 orpheline** + check:schema **158** + check:stripe + check:alt **28** + check:channels **25** + check:overflow **(11 pages)** + check:cookies. Emails et closing PDF vérifiés en capture. **Point d'arrêt : passe superviseur page par page avant merge.** |
