...
// ————————————————————————————————————————————————————————————————
// Désinscription : jeton HMAC (email signé, sans expiration : un lien de
// désinscription doit toujours fonctionner). base64url({ e, s }).
// ————————————————————————————————————————————————————————————————
...
export function unsubToken(email) {
  ...
  return Buffer.from(JSON.stringify({ e, s: signEmail(e) })).toString("base64url");
}

export function verifyUnsub(token) {
  try {
    const { e, s } = JSON.parse(Buffer.from(String(token), "base64url").toString());
    const expected = signEmail(e);
    const ok =
      s &&
...
export const unsubUrlFor = (email) =>
  `${SITE}/api/lead-unsubscribe?token=${unsubToken(email)}`;
...
// l'email côté Resend : pas besoin de cron pour la relance J+3.
// ————————————————————————————————————————————————————————————————
