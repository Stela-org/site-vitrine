...
// `export` ajoute au LOT DIAG-1 : le rapport de diagnostic reutilise la MEME
// enveloppe que les emails du guide, plutot que d'en dupliquer une seconde qui
// deriverait ensuite. Aucun changement pour les appelants existants.
export function shell({ preheader, firstName, bodyHtml, unsubUrl }) {
...
// LOT VIT-17 §1 — `scheduledId` (id Resend de la relance J+3) est de nouveau
// transmis au CRM, pour une raison DIFFÉRENTE de celle qui l'avait fait poser
// puis retirer.
//
// Historique : le commit a1007f4 l'envoyait pour permettre l'annulation à la
// désinscription ; 8c02565 l'a retiré, à juste titre, parce que ce chemin-là
// n'en a pas besoin (l'id revient par le jeton signé du lien de désinscription,
// cf. `verifyUnsub`). Ce retrait reste valable et n'est pas défait.
//
// Le besoin qui l'exige aujourd'hui est ailleurs : quand un lead est VERSÉ EN
// PROSPECTION côté CRM (lot CRM-2, `lib/admin/leadsToProspects.ts`), il sort du
// nurturing vitrine et doit voir sa relance planifiée annulée, sans quoi il
// reçoit deux séquences en parallèle. Cet endroit-là n'a aucun jeton : l'id ne
// peut lui venir que du payload. La migration 073 a posé la colonne, la
// validation l'accepte, l'annulation est écrite et attend — il ne manquait que
// cette ligne, côté vitrine.
//
// Omis quand la planification a échoué : le CRM lit « rien à annuler », ce qui
// est exact.
// `source` ajoutee au LOT DIAG-1, avec le defaut historique : le diagnostic est
// un second canal d'acquisition, et un lead qu'on ne sait pas d'ou il vient est
// un lead qu'on relancera mal. `marketing_leads.source` est une chaine libre
// cote app (aucune contrainte d'enumeration), le defaut garde les appelants
// existants a l'identique.
export function pushLeadToCrm({ email, firstName, consentedAt, scheduledId, source = "guide-google" }) {
  const payload = {
    email,
  ...
  if (scheduledId) payload.nurturing_scheduled_email_id = scheduledId;
  return crmPost("/api/leads", payload);
}
