...
export function ceQueStelaChange(constats, m, maintenant = new Date()) {
  ...
  if (lignes.length === 0) {
    return {
      ...
      lignes: [],
      // LOT DIAG-2b — LA FICHE SAINE AVAIT UN CONSTAT, PAS UNE RAISON D'AGIR.
      // Le texte s'arrêtait à « Stela automatise ce que vous faites à la
      // main » : vrai, et sans effet sur quelqu'un qui va bien, puisque ce
      // qu'il fait à la main marche déjà.
      //
      // La suite est en registre SÉCURISATION, jamais menace, et la nuance
      // tient à qui est le sujet : ce sont les circonstances ordinaires d'un
      // commerce qui font ralentir la mécanique (une équipe qui change, une
      // période chargée), pas une sanction ni un danger que nous
      // annoncerions. Aucun défaut n'est inventé : la phrase parle du futur
      // possible de n'importe quelle fiche, pas de celle-ci.
      //
      // ET ELLE NE PROMET RIEN. Pas de note, pas de classement, pas de
      // pourcentage : uniquement ce que Stela FAIT tous les jours. C'est la
      // règle que `scripts/check-diagnostic.mjs` fait respecter, et que les
      // CGV imposent trois pages plus loin (« aucune garantie de résultat
      // chiffré »).
      //
      // UNE SEULE CHAÎNE, à dessein : la page (`diagnostic.astro`) et le
      // rapport email (`api/diagnostic-rapport.js`) lisent tous deux ce même
      // `entretien`. Deux copies auraient divergé au premier ajustement.
      entretien: "Votre fiche est déjà à jour : Stela automatise ce que vous faites aujourd'hui à la main. Et une réputation n'est jamais acquise : un changement d'équipe, une période chargée, un concurrent qui s'installe, et la mécanique qui vous a menés ici ralentit sans prévenir. Stela la fait tourner tous les jours, même quand vous n'avez pas le temps d'y penser : chaque client invité, chaque avis répondu, votre fiche tenue à jour. Vous gardez l'avance que vous avez déjà prise.",
      arithmetique: null,
    };
  }
...
// ── LOT APERCU-1 §D : « Et dans les IA ? » ─────────────────────────────────
//
// La vitrine ne sait rien interroger toute seule : les moteurs, les clés et
// les plafonds vivent dans l'app (`/api/public/geo-preview`). Ce module ne
// fait que l'appeler et rapporter ce qu'elle répond.
//
// L'ORIGINE EST ENVOYÉE EXPLICITEMENT. C'est un appel serveur à serveur : il
// n'y a pas de navigateur pour poser l'en-tête `Origin`, et la route app
// n'accepte que nos deux domaines. On dit donc pour qui l'on appelle. Ce
// n'est pas une garde de sécurité (rien n'empêche un tiers de poser le même
// en-tête) : c'est ce qui empêche un autre site de faire payer nos appels à
// ses visiteurs, et l'app porte ses vrais plafonds par IP et par jour.
//
// L'URL VIENT DE L'ENVIRONNEMENT, jamais d'une constante : le domaine de
// l'app change entre la préproduction et la production, et une URL écrite en
// dur ferait pointer une preview vers la prod sans que personne le voie.
const APP_ORIGIN = process.env.APP_ORIGIN || "";
const ORIGINE_DECLAREE = "https://www.mystela.fr";
const APERCU_TIMEOUT_MS = 20_000;

/**
 * L'aperçu IA d'une fiche, ou `null`.
 *
 * `null` VEUT DIRE « INDISPONIBLE », ET RIEN D'AUTRE. Pas « pas cité », pas
 * un tableau vide qu'on afficherait comme un résultat. Le visiteur verra
 * « Indisponible pour le moment », qui est la vérité : nous n'avons pas su
 * mesurer. Inventer un « pas cité » à la place serait annoncer une mauvaise
 * nouvelle fausse à quelqu'un qui découvre le produit.
 *
 * Aucun échec ne remonte au visiteur autrement : le diagnostic Google est déjà
 * affiché et n'a pas à tomber parce qu'une IA n'a pas répondu.
 */
export async function apercuIa(placeId) {
  if (!APP_ORIGIN) return null;
  const ctrl = new AbortController();
  const minuteur = setTimeout(() => ctrl.abort(), APERCU_TIMEOUT_MS);
  try {
    const res = await fetch(`${APP_ORIGIN}/api/public/geo-preview`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: ORIGINE_DECLAREE },
      body: JSON.stringify({ placeId }),
      signal: ctrl.signal,
    });
    if (!res.ok) return null;
    const charge = await res.json();
    if (!charge || !Array.isArray(charge.engines) || !charge.query) return null;
    return { query: String(charge.query), measuredAt: String(charge.measuredAt || ""), engines: charge.engines };
  } catch {
    // Timeout, DNS, app en train de redéployer : tout finit ici, et tout dit
    // la même chose au visiteur.
    return null;
  } finally {
